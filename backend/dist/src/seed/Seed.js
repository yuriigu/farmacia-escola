...
async function main() {
    ...
    const farmPass = await bcryptjs_1.default.hash('farm123', 10);
    const medPass = await bcryptjs_1.default.hash('medico123', 10);
    const alunoPass = await bcryptjs_1.default.hash('aluno123', 10);
    const pacPass = await bcryptjs_1.default.hash('paciente123', 10);
    const admin = await prisma.user.create({
        data: { name: 'Admin Sistema', email: 'admin@farmaciaescola.edu.br', password: adminPass, role: Enums_1.Role.ADMIN, active: true, registerDoc: 'CRF/SP 00001' },
    });
    const farm1 = await prisma.user.create({
        data: { name: 'Farm. Luciana Mendes', email: 'luciana@farmaciaescola.edu.br', password: farmPass, role: Enums_1.Role.FARMACEUTICO, active: true, registerDoc: 'CRF/SP 12345' },
    });
    const farm2 = await prisma.user.create({
        data: { name: 'Farm. Pedro Almeida', email: 'pedro@farmaciaescola.edu.br', password: farmPass, role: Enums_1.Role.FARMACEUTICO, active: true, registerDoc: 'CRF/SP 12346' },
    });
    const medico = await prisma.user.create({
        data: { name: 'Dr. Roberto Santos', email: 'roberto.medico@farmaciaescola.edu.br', password: medPass, role: Enums_1.Role.MEDICO, active: true, registerDoc: 'CRM/SP 98765' },
    });
    const aluno = await prisma.user.create({
        data: { name: 'Ana Souza (Aluna)', email: 'ana.aluna@farmaciaescola.edu.br', password: alunoPass, role: Enums_1.Role.ALUNO, active: true, registerDoc: 'RA 2024001' },
    });
    const pacUser = await prisma.user.create({
        data: { name: 'João Silva', email: 'joao@email.com', password: pacPass, role: Enums_1.Role.PACIENTE, active: true },
    });
