...
class PatientService {
    ...
    async getAll(role, userId, search) {
        if (role === 'PACIENTE') {
    ...
    async create(userId, role, data) {
        ...
        if (isStaff) {
            await this.logService.log(userId, 'create', 'patients', patient.id, `Cadastrou paciente ${patient.name} (CPF: ${patient.cpf})`);
        }
        return patient;
    }
    async update(userId, role, id, data) {
        ...
        if (isStaff) {
            await this.logService.log(userId, 'update', 'patients', id, `Atualizou dados do paciente ${updated.name}`);
        }
        return updated;
    }
    async delete(userId, role, id) {
        ...
        if (isStaff) {
            await this.logService.log(userId, 'delete', 'patients', id, `Excluiu paciente ${existing.name}`);
        }
