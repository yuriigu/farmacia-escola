...
class AppointmentService {
    ...
    logService;
    constructor() {
        this.appointmentRepo = new AppointmentRepository_1.AppointmentRepository();
    ...
    async getAll(role, userId, patientId) {
        if (role === 'PACIENTE') {
    ...
    async create(user, data) {
        ...
        if (numericSlotId) {
            ...
            const currentCount = await Prisma_1.prisma.appointment.count({ where: { slotId: numericSlotId } });
        ...
        await this.logService.log(user.userId, 'create', 'appointments', appointmentLogId, `Criou agendamento de dispensação para paciente #${targetPatientId}`);
        return appointment;
    }
    async updateStatus(userId, role, id, status, notes) {
        const numericId = Number(id);
        if (!numericId) {
            throw { statusCode: 400, message: 'ID de agendamento inválido' };
        }
        ...
        await this.logService.log(userId, 'update_status', 'appointments', numericId, `Atualizou status do agendamento #${numericId} para ${normalizedStatus}`);
        return updated;
    }
    async update(userId, role, id, data) {
        ...
        await this.logService.log(userId, 'update', 'appointments', numericId, `Atualizou agendamento #${numericId}`);
        return updated;
    }
    async delete(userId, role, id) {
        ...
        await this.logService.log(userId, 'delete', 'appointments', numericId, `Cancelou/excluiu agendamento #${numericId}`);
        return { message: 'Agendamento cancelado/excluído com sucesso' };
    }
    async calculateRealAvailableStock(medicineId) {
        ...
        let activeBatches = [];
        try {
            activeBatches = await Prisma_1.prisma.stockBatch.findMany({
                where: {
        ...
        if (physicalStockTotal === 0) {
            if (med) {
                ...
                if (!batchesFound) {
                    if (typeof med.totalQuantity === 'number') {
                        if (med.totalQuantity > 0) {
                            physicalStockTotal = med.totalQuantity;
                        }
                    }
                }
            }
