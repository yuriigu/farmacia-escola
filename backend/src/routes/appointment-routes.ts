...
router.post('/:id/dispense', requireAnyPermission('APPOINTMENTS_UPDATE', 'WITHDRAWALS_CREATE'), authorizeRoles('ADMIN', 'FARMACEUTICO', 'ALUNO'), controller.dispense);
router.post('/:id/revert-dispense', requireAnyPermission('APPOINTMENTS_UPDATE', 'WITHDRAWALS_CANCEL'), authorizeRoles('ADMIN', 'FARMACEUTICO'), controller.revertDispense);
